...
<div class="grid">
  ...
  <div>
    ...
    <details class="methods"><summary>Methodology &amp; equations</summary>
      ...
      measured against):</p>
      <table class="attr">
        <tr><th>product</th><th>price (&times; conv.)</th><th>taste (1 = real meat)</th><th>slaughter&#8209;free</th><th>real tissue</th></tr>
        <tr><td><b>conventional</b> meat</td><td>1 (anchor)</td><td>1 (reference)</td><td>no</td><td>yes</td></tr>
        <tr><td><b>plant&#8209;based</b> meat</td><td>\(R_p\) (~1.77, dial)</td><td>\(a_p\) (~0.8, dial)</td><td>yes</td><td>no</td></tr>
        <tr><td><b>cultivated</b> meat</td><td>\(R\) (from §1)</td><td>\(a_x\) (~1, dial)</td><td>yes</td><td>yes</td></tr>
        <tr><td><b>whole&#8209;food</b> (beans/tofu)</td><td>~0.25 (cheap)</td><td>~0.3 (not meat)</td><td>yes</td><td>no</td></tr>
      </table>
      ...
      plant-based <i>meat</i> sits at only ~1%. Every product runs through the <b>same</b> utility rule
      (no option gets a special term):</p>
      \[ V_j = \underbrace{\alpha\ln(y-\text{price}_j)}_{\text{price, via income}}
              ...
              \;+\; q\,\text{taste}_j \;+\; w^{s}\,\text{free}_j \;+\; w^{rt}\,\text{tissue}_j \;+\; \text{ASC}_j \]
      ...
      <p>Six ingredients, each with a plain meaning and a source:</p>
      <table class="vardef">
        <tr><td><b>price &amp; income</b> \(\alpha\ln(y-\text{price})\)</td><td>the <b>Berry–Levinsohn–Pakes
          (1995)</b> way to put income in: <b>\(y\)</b> is the buyer's income (the <b>income</b> slider) and
          <b>price</b> is the product's $/kg, so \(y-\text{price}\) is what's left after buying it — and the
          \(\ln\) makes a dollar matter <i>less</i> to a richer person. <b>\(\alpha\)</b> is the price
          coefficient (how sharply people flee price); we don't guess it — it is pinned to the measured meat
          elasticity \(\varepsilon\) (&minus;0.9, scanner data) at a reference income, <i>steepened</i> by
          \(\kappa\) because cultivated has a near-perfect substitute (conventional). Concretely the price
          coefficient is \(\beta=\varepsilon\,\kappa/\big(p_{\rm calib}(1-s)\big)\) and \(\alpha=-\beta\,(y_{\rm
          ref}-p_{\rm calib})\): \(\kappa\) simply <i>multiplies</i> the measured meat elasticity. Why \(\approx
          3\)? — there is no cultivated cross-price data, so this is a judgement, not a measurement: meat
          <i>as a category</i> is inelastic (\(\varepsilon\approx-0.9\), no close substitute), but cultivated has
          one (real conventional tissue), so its <i>own</i>-price response is several times larger. The retired
          nested-logit version implied \(\approx 2\times\); we centre at \(3\) and treat \(2\!-\!4\) as the range.
          It is the least data-disciplined number in the model and one of the two biggest demand levers (with
          \(\lambda\)) at the likely \(R\approx2.4\) — which is why it is a slider, not a buried constant. So
          raising \(y\) (a richer country, or a high-growth future) flattens everyone's price sensitivity. (Across regions
          \(y\) enters with a mild damping, so the rich→poor sensitivity gap matches the ~2–3&times; seen
          in the data rather than a raw 1/income. <b>Price basis:</b> we use the per-kg shelf price — the
          basis the elasticity \(\varepsilon\) is measured on. Using a monthly or annual meat budget instead
          only rescales \(\alpha\), which the calibration absorbs, so the shares are unchanged: meat is cheap
          relative to income at any basis, so the \(\ln\) is essentially linear.)</td></tr>
        ...
        <tr><td><b>ASC</b> (the leftover)</td><td>a per-product constant for any baseline appeal the four
          attributes miss — the standard "alternative-specific constant" of discrete choice. Here it is
          <b>zero for conventional and plant-based</b> (their attributes already describe them); only two
          carry one: <b>whole-food</b>'s is the single calibration knob (solved so the model matches
          plant-based's real share — see below), and <b>cultivated</b>'s is its novelty-standing plus the
          per-meat-type tier resistance (§3). So it isn't a free fudge — it's two named, justified numbers.</td></tr>
      </table>
      ...
      data anywhere. The retired nested-logit version implied \(\approx2\times\); we centre at \(3\) and treat
      \(2\!-\!4\) as the range. Drag the slider to \(\kappa=1\) ("cultivated as inelastic as meat overall") and
...
/* TWO-SEGMENT, FOUR-PRODUCT discrete-choice (logit) demand — a line-for-line mirror of
   ...
   aversion premium penalty, taste, slaughter-free, real-tissue, and an alternative-specific
   constant (ASC). Total share = w_eth*P_ethical + (1-w_eth)*P_mainstream. K = the effective
   constants (sliders override C) WITH the three solved values attached by solveCalibration(). */
...
function utilities(R,K,seg,o){
  ...
  const ascW=(seg==="M")?K.K_wholefood_M:K.K_wholefood_E;           // segment-specific whole-food ASC
  const asc=[ascW,0,0,o.toff];                                      // cultivated ASC = the standing/tier offset
  ...
  for(let j=0;j<4;j++){
    ...
    V[j]=Vp+Vl+K.q_taste*taste[j]+wSl*slaughter[j]+wRt*realtissue[j]+asc[j];
  }
  return V;
}
