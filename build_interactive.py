...
<div class="grid">
  ...
  <div>
    ...
    <details class="methods"><summary>Methodology &amp; equations</summary>
      ...
      <p>The biomass cost carries a second, deeper floor \(c_{\rm floor}=\underbrace{a_{\rm I}\,p_{\rm
      aa}}_{\text{amino acids}}+\underbrace{g}_{\text{glucose}}+\underbrace{h_{\rm min}}_{\text{minimal plant}}\!
      \approx\$7.5\)/kg — the irreducible feedstock the cells must physically eat plus the floor cost of running a
      plant at scale, assuming the reducible parts (recombinant proteins, single-use consumables, small-scale
      capital) are engineered toward zero. It is the "where is the floor?" number, though in the reachable slider
      range \(c_{\rm med}+h\) already sits at or above it, so the \(\max\) rarely binds. Parity (\(R_x=1\))
