...
REGISTRY: dict[str, Input] = {
    ...
    "loss_aversion": Input(1.0, "ratio", "REFERENCE-DEPENDENT price asymmetry lambda (OFF by default). Consumers "
        ...
        note="WHY DEFAULT 1.0 (symmetric, no kink) rather than the TK 2.25: (1) it is NEAR-INERT on the headline "
             ...
             "penetration roll-up UNCHANGED). CAVEAT: this absorption holds only up to lambda~2.6, where beta "
             "saturates at its monotonicity cap (beta_cap = 1/p_conv - 1e-3); ABOVE that the realised own-price "
             "elasticity steepens past the eps_own*kappa target (e.g. ~-7 at lambda=4 vs the -3.6 target) because "
             "the kink can no longer be compensated -- acceptable for an OFF-by-default exploratory dial, but the "
             "'only reshapes the kink' invariant is a low-lambda statement, not global; (2) Bell & Lattin 2000 "
             "(Marketing Sci 19:185) show estimated loss "
